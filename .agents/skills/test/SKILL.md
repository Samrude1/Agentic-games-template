...
- Run the tests locally using terminal commands (e.g. `npm test` or `node`).
- Never claim tests pass without empirical execution evidence.
- If a test fails, analyze the error:
  - If the game logic is wrong, fix the logic at the root cause.
